...
namespace TradeWeb.API.Repository
{
    public class UtilityCommon : IUtilityCommon
    {
        ...
        public DataSet TradeHoldingAson(string rsCon, string strClient, string strdate, string strcollat, SqlConnection ObjConnectionTmp)
        {
            SqlConnection con;
            using (var db = new DataContext())
            {
            ...
            strRMSVALATLTRT = GetSysParmSt("RMSVALATLTRT", "").ToUpper().Trim();
            try
            { ExecuteSQLTmp("Create Table #TmpRates (dm_scripcd VarChar(6), dm_bserate money )", con); }
            catch (Exception e)
            {
                ExecuteSQLTmp("Drop table  #TmpRates", con);
                Strsql = "Create Table #TmpRates(dm_scripcd VarChar(6), dm_bserate money)";
                ExecuteSQLTmp(Strsql, con);
            ...
            {
                ExecuteSQLTmp("Drop table  #PreserveHld", con);
            ...
            Strsql += " where dm_stlmnt = se_stlmnt and dm_clientcd = cm_cd and dm_scripcd = ss_cd and dm_Dt <= '" + strdate + "'";
            ...
            Strsql += " and ( dm_execdt > '" + strdate + "' Or IsNull(dm_execdt,0) = 0 ) and dm_dt <= '" + strdate + "'";
            Strsql += " and cm_cd='" + strClient + "' ";
            Strsql += " group by dm_clientcd,dm_scripcd,dm_isin, ss_name,se_stlmnt";
            Strsql += " Having sum(dm_qty)<>0";
            ExecuteSQLTmp(Strsql, con);
            ...
            if (DsMPTrx.Tables[0].Rows.Count > 0)
            {
                ...
                Strsql += " and MPT_clientcd='" + strClient + "' ";
                ...
                Strsql += " and MPT_scripcd = im_scripcd and ss_cd = MPT_scripcd ";
                Strsql += " and MPT_OurDP in (" + strOurdp + ") ";
                Strsql += " and MPT_TRXFlag = 'R' and MPT_clientcd='" + strClient + "' ";
                Strsql += " group by MPT_clientcd,MPT_scripcd,im_isin, ss_name ";
            ...
            if (Conversion.Val(strRMSVALATLTRT) > 0)
            {
                ...
                DateTime DtRMSVALATLTRT = ConvertDT(strdate).AddDays(-Conversion.Val(strRMSVALATLTRT));
                StrNewRMSVALATLTRT = DtRMSVALATLTRT.ToString("yyyyMMdd");
                Strsql += " and mk_dt >='" + StrNewRMSVALATLTRT + "'";
            }
            Strsql += " and mk_dt <='" + strdate + "' )";
            ExecuteSQLTmp(Strsql, con);
            ...
            Strsql = " select dm_scripcd, sum(dm_bcqty) dm_bcqty ,dm_isin,dm_Scripname, ";
            Strsql += " convert(decimal(15,2) ,dm_bserate) dm_bserate , convert(decimal(15,2), sum(dm_valuation)) dm_valuation,'' dm_Type";
            Strsql += " from #PreserveHld group by dm_scripcd,dm_isin,dm_Scripname,dm_bserate";

            ObjCommand.CommandText = Strsql;
            ObjCommand.Connection = con;
            ObjAdapter.SelectCommand = ObjCommand;
            ObjAdapter.Fill(ObjDataSet, "Summary");

            ObjCommand.CommandText = " select dm_scripcd,dm_stlmnt,dm_bcqty,dm_isin,dm_Scripname,convert(decimal(15,2) ,dm_bserate) dm_bserate ,convert(decimal(15,2),dm_valuation) dm_valuation,'' dm_Type from #PreserveHld";
            ObjCommand.Connection = con;
            ObjAdapter.SelectCommand = ObjCommand;
            ObjAdapter.Fill(ObjDataSet, "Detail");
        ...
        public DataSet TradeHolding(string rsCon, string strClient, string strdate, string strcollat, SqlConnection ObjConnectionTmp)
        {
            SqlConnection con;
            using (var db = new DataContext())
            {
            ...
            GetHairCut(con);
            Strsql = GetSqlTradeHolding(con.ConnectionString, "'" + strClient + "'", strdate, strcollat, con);
            ObjCommand.CommandText = Strsql;
            ObjCommand.Connection = con;
            ObjAdapter.SelectCommand = ObjCommand;
            ObjAdapter.Fill(ObjDataSet, "Summary");
            ...
            DataSet DsMPTrx = new DataSet();
            DsMPTrx = OpenDataSet("select * from SysObjects where name= 'MrgPledge_TRX'");
            Strsql = "select dm_clientcd,dm_scripcd,se_stlmnt,sum(dm_qty * (-1)) as qty,dm_isin,";
            Strsql = Strsql + " ss_name,ss_bserate,convert(decimal(15,2),(ss_bserate*sum(dm_qty * (-1)))) as valuation ,'Beneficiary Holding' as bh_type,cast((Tmp_haricut) as decimal(15,2)) HairCut,convert(decimal(15,2), 0) as AfterHaricut ,(case isNull(dm_tmkttype,'')  when 'X' then 'Marked For Sale against debit' else '' end)  as Remark ";
            Strsql = Strsql + " ,'' unpledge,'' lnkUnPledge from demat with (nolock), ourdps with (nolock) ,settlements with (nolock) ,client_master with (nolock),securities with (nolock),#TmpVarMargin";
            Strsql = Strsql + " where cm_cd = dm_clientcd and dm_stlmnt = se_stlmnt and dm_scripcd = ss_cd";
            Strsql = Strsql + " and dm_ourdp = od_cd";
            Strsql = Strsql + " and Tmp_scripcd = dm_scripcd and od_acttype in ('B','M') and dm_type = 'BC' and dm_locked = 'N' and dm_transfered = 'N'";
            Strsql = Strsql + " and dm_clientcd = '" + strClient.Trim() + "' and dm_ourdp not in " + strcollat;
            Strsql = Strsql + " group by dm_clientcd,dm_scripcd,dm_isin, ss_name,se_stlmnt,Tmp_haricut,ss_bserate,dm_tmkttype having abs(sum(dm_qty)) > 0 ";
            Strsql = Strsql + " union all ";

            Strsql = Strsql + " select dm_clientcd,dm_scripcd,se_stlmnt,sum(dm_qty * (-1)) as qty,dm_isin,ss_name,ss_bserate, convert(decimal(15,2),(ss_bserate*sum(dm_qty * (-1))))";
            Strsql = Strsql + " as valuation ,'COLLAT Holding' as bh_type,cast((Tmp_haricut) as decimal(15,2)) HairCut,convert(decimal(15,2), 0) as AfterHaricut ,(case isNull(dm_tmkttype,'')  when 'X' then 'Marked For Sale against debit' else '' end)  as Remark ";
            Strsql = Strsql + " ,'' unpledge,'' lnkUnPledge from demat with (nolock),ourdps with (nolock) ,settlements with (nolock) ,client_master with (nolock) ,securities with (nolock) ,#TmpVarMargin where cm_cd = dm_clientcd and dm_stlmnt = se_stlmnt and dm_scripcd = ss_cd and dm_ourdp = od_cd and Tmp_scripcd = dm_scripcd";
            Strsql = Strsql + " and od_acttype in ('B','M') and dm_type = 'BC' and dm_locked = 'N' and dm_transfered = 'N'";
            Strsql = Strsql + " and dm_clientcd = '" + strClient.Trim() + "' and dm_ourdp in " + strcollat;
            Strsql = Strsql + " group by dm_clientcd,dm_scripcd,dm_isin, ss_name,se_stlmnt,Tmp_haricut,ss_bserate,dm_tmkttype";
            Strsql = Strsql + " having abs(sum(dm_qty)) > 0  ";
            Strsql = Strsql + " Union all  ";

            Strsql = Strsql + " select dm_clientcd,dm_scripcd,se_stlmnt,sum(dm_qty * (-1)) as qty,dm_isin,ss_name,ss_bserate, convert(decimal(15,2),(ss_bserate*sum(dm_qty * (-1)))) as valuation ,'Expected Holding' as bh_type,cast((Tmp_haricut) as decimal(15,2)) HairCut,convert(decimal(15,2), 0) as AfterHaricut ,(case isNull(dm_tmkttype,'')  when 'X' then 'Marked For Sale against debit' else '' end)  as Remark ";
            Strsql = Strsql + " ,'' unpledge,'' lnkUnPledge from demat with (nolock), ourdps with (nolock) ,settlements with (nolock) ,client_master with (nolock) ,securities  with (nolock) ,#TmpVarMargin";
            Strsql = Strsql + " where cm_cd = dm_clientcd and dm_stlmnt = se_stlmnt and dm_scripcd = ss_cd and dm_ourdp = od_cd and od_acttype = 'P'and dm_type = 'BC' and dm_locked = 'N' and dm_transfered <> 'S' and se_shpayoutdt > '" + strdate + "'";
            Strsql = Strsql + " and dm_ourdp not in " + strcollat;
            Strsql = Strsql + " and dm_clientcd = '" + strClient.Trim() + "' and Tmp_scripcd = dm_scripcd";
            Strsql = Strsql + " group by dm_clientcd,dm_scripcd,dm_isin, ss_name,se_stlmnt,Tmp_haricut,ss_bserate,dm_tmkttype";
            Strsql = Strsql + " having abs(sum(dm_qty)) > 0  ";
            Strsql = Strsql + " Union all  ";
            ...
            Strsql = Strsql + " where cm_cd = dm_clientcd and dm_stlmnt = se_stlmnt and dm_scripcd = ss_cd and dm_ourdp = od_cd and od_acttype = 'P' and dm_type = 'CB' and dm_locked = 'N' and dm_transfered <> 'S'";
            Strsql = Strsql + " and dm_clientcd = '" + strClient.Trim() + "' and dm_ourdp not in " + strcollat;
            Strsql = Strsql + " and Tmp_scripcd = dm_scripcd group by dm_clientcd,dm_scripcd,dm_isin, ss_name,se_stlmnt,Tmp_haricut,ss_bserate,dm_tmkttype having abs(sum(dm_qty)) > 0 ";

            Strsql = Strsql + " union all ";
            Strsql = Strsql + " select dm_clientcd,dm_scripcd,se_stlmnt,sum(dm_qty * (-1)) as qty,dm_isin,";
            Strsql = Strsql + " ss_name,ss_bserate,convert(decimal(15,2),(ss_bserate*sum(dm_qty * (-1)))) as valuation ,(case od_acttype when 'T' then 'Margin Trading' when 'C' then 'MTF Collateral' else '' end ) as bh_type,cast((Tmp_haricut) as decimal(15,2)) HairCut,convert(decimal(15,2), 0) as AfterHaricut ,(case isNull(dm_tmkttype,'')  when 'X' then 'Marked For Sale against debit' else '' end)  as Remark ";
            Strsql = Strsql + " ,'' unpledge,'' lnkUnPledge from demat with (nolock), ourdps with (nolock) ,settlements with (nolock) ,client_master with (nolock),securities with (nolock),#TmpVarMargin";
            Strsql = Strsql + " where cm_cd = dm_clientcd and dm_stlmnt = se_stlmnt and dm_scripcd = ss_cd";
            Strsql = Strsql + " and dm_ourdp = od_cd";
            Strsql = Strsql + " and Tmp_scripcd = dm_scripcd and od_acttype in ('T','C') and dm_type = 'BC' and dm_locked = 'N' and dm_transfered = 'N'";
            Strsql = Strsql + " and dm_clientcd = '" + strClient.Trim() + "'";
            Strsql = Strsql + " group by dm_clientcd,dm_scripcd,dm_isin, ss_name,se_stlmnt,Tmp_haricut,ss_bserate,dm_tmkttype,od_acttype";
            Strsql = Strsql + " having abs(sum(dm_qty)) > 0 ";
            ...
            if (DsMPTrx.Tables[0].Rows.Count > 0)
            {
                ...
                Strsql = Strsql + " union all ";
                Strsql = Strsql + " select MPT_clientcd dm_clientcd,MPT_scripcd dm_scripcd,'',sum(case When MPT_DRCR ='C' Then MPT_Qty else -MPT_Qty  end) as qty,im_isin,ss_name,ss_bserate, (ss_bserate*sum(case When MPT_DRCR ='C' Then MPT_Qty else -MPT_Qty  end)) as valuation ,'Margin Pledge' bh_type, ";
                Strsql = Strsql + " cast((Tmp_haricut) as decimal(15,2)) HairCut,  convert(decimal(15,2), 0) as AfterHaricut ,'' ";
                Strsql = Strsql + " ,'Un-Pledge ' + isNull((select case when Rq_Qty > 0 then ' [' + convert(varchar,Rq_Qty) +'] Qty' else '' end from PledgeRequest where Rq_Status1='X' and Rq_Status3 ='P' and Rq_Clientcd=MPT_clientcd and Rq_Scripcd=MPT_scripcd),'') unpledge,";
                Strsql = Strsql + " MPT_clientcd +'/'+ MPT_scripcd +'/' + ss_name +'/'+ convert(varchar,sum(case When MPT_DRCR ='C' Then MPT_Qty else -MPT_Qty  end)) + '/P' lnkUnPledge ";
                Strsql = Strsql + " from MrgPledge_TRX,securities with (nolock),#TmpVarMargin ,Isin with (nolock)  ";
                Strsql = Strsql + " where im_scripcd = ss_cd and im_active = 'Y' and im_priority = (select min(im_priority) from Isin Where im_scripcd = Tmp_scripcd and im_active = 'Y' ) and Tmp_scripcd = im_scripcd  and  Tmp_scripcd = MPT_scripcd ";
                Strsql = Strsql + " and MPT_clientcd = '" + strClient.Trim() + "' ";
                Strsql += " and MPT_OurDP in (" + strOurdp + ") ";
                Strsql = Strsql + " group by MPT_clientcd,MPT_scripcd,im_isin,ss_name,ss_bserate,Tmp_haricut ";
                Strsql = Strsql + " having abs(sum(case When MPT_DRCR ='C' Then MPT_Qty else -MPT_Qty  end)) > 0 ";
                Strsql = Strsql + " union all ";
                Strsql = Strsql + " select MPT_clientcd dm_clientcd,MPT_scripcd dm_scripcd,'',Sum(case MPT_DRCR when 'D' Then MPT_Qty else -MPT_Qty end) as qty,im_isin,ss_name,ss_bserate, (ss_bserate*sum(case When MPT_DRCR ='C' Then MPT_Qty else -MPT_Qty  end)) as valuation ,'Margin Pledge' bh_type, ";
                Strsql = Strsql + " cast((Tmp_haricut) as decimal(15,2)) HairCut,  convert(decimal(15,2), 0) as AfterHaricut ,'' ";
                Strsql = Strsql + " ,'Un-Pledge ' + isNull((select case when Rq_Qty > 0 then ' [' + convert(varchar,Rq_Qty) +'] Qty' else '' end from PledgeRequest where Rq_Status1='X' and Rq_Status3 ='R' and Rq_Clientcd=MPT_clientcd and Rq_Scripcd=MPT_scripcd),'') unpledge,";
                Strsql = Strsql + " MPT_clientcd +'/'+ MPT_scripcd +'/' + ss_name +'/'+ convert(varchar,sum(case When MPT_DRCR ='D' Then MPT_Qty else -MPT_Qty  end)) + '/P' lnkUnPledge ";
                Strsql = Strsql + " from MrgPledge_TRX,securities with (nolock),#TmpVarMargin ,Isin with (nolock)  ";
                Strsql = Strsql + " where im_scripcd = ss_cd and im_active = 'Y' and im_priority = (select min(im_priority) from Isin Where im_scripcd = Tmp_scripcd and im_active = 'Y' ) and Tmp_scripcd = im_scripcd  and  Tmp_scripcd = MPT_scripcd ";
                Strsql = Strsql + " and MPT_TRXFlag = 'R'  and MPT_clientcd = '" + strClient.Trim() + "' ";
                Strsql += " and MPT_OurDP in (" + strOurdp + ") ";
                Strsql = Strsql + " group by MPT_clientcd,MPT_scripcd,im_isin,ss_name,ss_bserate,Tmp_haricut ";
                Strsql = Strsql + " having abs(Sum(case MPT_DRCR when 'D' Then MPT_Qty else -MPT_Qty end)) > 0 ";
            }
            ...
            ObjDataSet.Tables[0].Columns.Add("SubQty");
            ObjDataSet.Tables[0].Columns.Add("SubVal");
